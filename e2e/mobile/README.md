...
1. Start the API in explicit test mode using a LAN-reachable host and an isolated database. Set `APP_TEST_MODE=true`, `AUTH_REQUIRED=true`, `DATABASE_URL` to a local database ending in `_e2e`, `OAUTH_REDIRECT_BASE_URL` to the API's LAN URL, and `WEB_APP_URL` to the web test URL.
