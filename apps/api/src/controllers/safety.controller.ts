...
import type { ResponseFeedbackRequest, UnsafeOutputReportRequest } from "@persona/shared";
import { and, eq } from "drizzle-orm";
import type { Request, Response } from "express";
import { getDatabase } from "../db/client.js";
import { conversations, responseFeedback, unsafeOutputReports } from "../db/schema.js";
...
export async function postResponseFeedback(request: Request, response: Response): Promise<void> {
  const userId = requestAuthenticatedOwnerId(request);
  const body = request.body as ResponseFeedbackRequest;
  const database = getDatabase();
  if (!database) throw new HttpError("Feedback is temporarily unavailable.", 503);
  ...
  const createdAt = new Date();
  await database.insert(responseFeedback).values({
    id,
    userId,
    conversationId: conversation.id,
    category: body.category,
  ...
  response.status(201).json({ feedback: { id, status: "received", createdAt: createdAt.toISOString() } });
}
