...
const envSchema = z.object({
  ...
  // Includes both visible answer tokens and hidden reasoning tokens. Web
  // search can consume a substantial reasoning budget before writing text.
  ...
  OPENAI_MAX_CONTEXT_CHARACTERS: z.coerce.number().int().min(1000).default(50000),
  OPENAI_MAX_CONTEXT_TOKENS: z.coerce.number().int().min(500).default(12000),
  CONVERSATION_MEMORY_SUMMARY_ENABLED: z.preprocess(stringToBoolean, z.boolean().default(true)),
  CONVERSATION_MEMORY_SUMMARY_AFTER_MESSAGES: z.coerce.number().int().min(4).max(200).default(24),
  ...
  GEMINI_NATIVE_YOUTUBE_REQUEST_TIMEOUT_MS: z.coerce.number().int().positive().default(120000),
  GEMINI_VIDEO_ANALYSIS_ENABLED: z.preprocess(stringToBoolean, z.boolean()).default(true),
  GEMINI_VIDEO_ANALYSIS_CACHE_TTL_MS: z.coerce.number().int().positive().default(604800000),
  GEMINI_VIDEO_ANALYSIS_AUTO_MAX_DURATION_SECONDS: z.coerce.number().int().positive().default(3600),
  GEMINI_VIDEO_ANALYSIS_EXPLICIT_MAX_DURATION_SECONDS: z.coerce.number().int().positive().default(10800),
  GEMINI_VIDEO_ANALYSIS_MAX_OUTPUT_TOKENS: z.coerce.number().int().min(256).max(4096).default(1600),
  ...
  GEMINI_OUTPUT_COST_PER_MILLION: z.coerce.number().nonnegative().default(2.5),
  ...
  ADMOB_ANDROID_REWARDED_AD_UNIT_ID: z.preprocess(optionalTrimmedString, z.string().regex(/^ca-app-pub-\d{16}\/\d{10}$/).optional()),
  ADMOB_IOS_REWARDED_AD_UNIT_ID: z.preprocess(optionalTrimmedString, z.string().regex(/^ca-app-pub-\d{16}\/\d{10}$/).optional()),
  ADMOB_SSV_PUBLIC_KEYS_URL: z.string().url().default("https://www.gstatic.com/admob/reward/verifier-keys.json"),
  ADMOB_SSV_MAX_AGE_MS: z.coerce.number().int().min(60_000).max(24 * 60 * 60 * 1000).default(24 * 60 * 60 * 1000),
  ...
  CHAT_DEVICE_RATE_LIMIT_REQUESTS: z.coerce.number().int().positive().default(45),
  ...
  DATA_TRANSFER_RATE_LIMIT_WINDOW_MS: z.coerce.number().int().min(60_000).max(24 * 60 * 60 * 1000).default(15 * 60 * 1000),
  ...
  DATA_TRANSFER_ARCHIVE_MAX_BYTES: z.coerce.number().int().min(25 * 1024 * 1024).max(5 * 1024 * 1024 * 1024).default(5 * 1024 * 1024 * 1024),
  DATA_TRANSFER_JSON_MAX_BYTES: z.coerce.number().int().min(1024 * 1024).max(512 * 1024 * 1024).default(128 * 1024 * 1024),
  DATA_TRANSFER_PRESIGNED_UPLOAD_TTL_SECONDS: z.coerce.number().int().min(300).max(24 * 60 * 60).default(5 * 60 * 60),
  DATA_TRANSFER_JOB_TTL_HOURS: z.coerce.number().int().min(1).max(168).default(24),
  ...
  TERMS_POLICY_VERSION: z.preprocess(optionalTrimmedString, z.string().min(1).max(80).default("2026-09-07.1")),
  PRIVACY_POLICY_VERSION: z.preprocess(optionalTrimmedString, z.string().min(1).max(80).default("2026-09-07.1")),
  ...
  GOOGLE_MAPS_API_KEY: z.preprocess(optionalWhitespaceFreeSecret, z.string().min(1).optional()),
  PLACES_SEARCH_ENABLED: z.preprocess(stringToBoolean, z.boolean().default(false)),
  PLACES_SEARCH_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(30_000).default(10_000),
